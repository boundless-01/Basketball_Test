...
void main_loop(void)
{
	if (timer_5ms_Flag) // 5ms定时器
	{
		...
		static int mode = 0;                          //运动模式标志位
		static int setballOK = 0;                     //捡球阶段运行成功标志位
		static int grabflag = 0;                      //抓球
		static int freeflag = 0;                      //扔球
		
		static float expPos = 0;
		static float expVel = 0;
		
		float tarPos = -630.f / 180.f * PI;           //目标位置  -120.f / 360.f * 4096.f * 6.f
		...
		float setoutposTime = 4.0f;                   //运动到捡球位置控制周期 4.0f
    float speedDownTime = 1.0f;		                //减速控制周期 0.5f
		
		uint32_t p_pos = 0.1 * 60.f * 1000.f;         //位置环KP 1.5 * 60.f * 1000.f
		uint32_t d_pos = 30.0 * 0.1f * 1000.f;        //位置环KD 0.2 * 0.1f * 1000.f
		
    uint32_t p_speed =  2.2 * 1000.f;             //速度环KP 3.5*1000.f
		uint32_t i_speed =  0.1 * 1000.f;             //速度环KI 3.0*1000.f
...
//		DEBUG("%d, %f, %f, %f, %f, %f\r\n", mode, upperArmMsg.pos / 6.f / PI * 180.f, upperArmMsg.pos, expVel / 4096.f, upperArmMsg.vel / 2.f / PI, upperArmMsg.torque);
			DEBUG("%d, %f, %f, %d, %f ,%f ,%f , %d\r\n", mode, expPos / 4096.f / 6.f * 2 * 180.f, upperArmMsg.pos / 6.f / PI * 180.f,
																							setballtim_cnt, expVel / 4096.f, upperArmMsg.vel / 2.f / PI, upperArmMsg.torque, freeflag);
			if(setballOK == 0)
			{
				...
				else if(setballtim_cnt == 600)
				{
					BEEP_OFF;
					ResetControlMode(&UP_FDCAN, PTP_MODE, UPPER_ARM_ID, VEL_MODE);
					SetVelLimit(&UP_FDCAN, PTP_MODE, UPPER_ARM_ID, 20 * 4096);
...
/*******************************机械臂操作************************************/
			if(mode == 1)      //捡球阶段
			{
				if(setoutpos_cnt <= setoutposTime / 0.005)
				{
					...
					expPos = 13.f / 360.f * 4096.f * 6.f * sin(PI * setoutpos_cnt / 800 / 2);
			...
			if(mode == 2)      //加速阶段
			{
				...
				{
					if(speed_up_cnt <= 0.05f / 0.005)
					{
						p_speed = 0.8 * 1000.f;
						i_speed = 0.1 * 1000.f;
					}
					else if (speed_up_cnt <= 0.15f / 0.005 && speed_up_cnt > 0.05f / 0.005)
					{
						p_speed = 2.5 * 1000.f;
						i_speed = 0.2 * 1000.f;
					}
